...
var getAllPasses = /*#__PURE__*/function () {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          _context.prev = 3;
          pass_no = searchParams.pass_no, status = searchParams.status; // Construct dynamic query object for PASS_COLLECTION
          query = {}; // Add search conditions for visitor details in the PASS_COLLECTION
          if (pass_no) {
            query.pass_no = {
              $regex: pass_no,
              $options: "i"
          ...
          validPerPage = Math.min(Math.max(1, perPage), 100); // Limit max results per page
          // Get the total count of passes that match the query
          _context.next = 12;
          return MongoDB.db.collection(mongoConfig.collections.PASS_COLLECTION).countDocuments(query);
        case 12:
          totalCount = _context.sent;
          _context.next = 15;
          return MongoDB.db.collection(mongoConfig.collections.PASS_COLLECTION).find(query).skip((validPage - 1) * validPerPage).limit(validPerPage).toArray();
        case 15:
          passes = _context.sent;
          // Extract visitorIds from the passes
          visitorIds = passes.map(function (pass) {
            return pass.visitorId;
          }); // Convert visitorIds to ObjectId to query the VISITER collection
          objectIds = visitorIds.map(function (id) {
            return ObjectId(id);
          }); // Fetch corresponding visitor details from the VISITER collection
          _context.next = 20;
          return MongoDB.db.collection(mongoConfig.collections.VISITER).find({
            _id: {
              $in: objectIds
            }
          }).toArray();
        case 20:
          ...
          passesWithVisitorDetails = passes.map(function (pass) {
            var visitor = visitors.find(function (visitor) {
              return visitor._id.toString() === pass.visitorId;
            });
            return _objectSpread(_objectSpread({}, pass), {}, {
        ...
        case 26:
          ...
          console.error('Error in getAllPassesWithVisitorDetails:', _context.t0);
          return _context.abrupt("return", {
            status: false,
